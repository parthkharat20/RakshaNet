...
import { Search, Filter, AlertTriangle, ShieldCheck, Lock, RefreshCw } from 'lucide-react';
...
export const AlertFeed = ({ onFreezeClick }) => {
  ...
  return (
    <div className="glass-panel flex flex-col h-full overflow-hidden border-white/10">
      ...
      <div className="p-4 border-b border-white/10 space-y-3 shrink-0">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-0.5">
              Dual-AI Pipeline Prioritization Stream ({alerts.length} Active)
            </p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by suspect name, account number, or ring..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-4 py-2 rounded-lg bg-slate-950/80 border border-white/10 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors font-mono"
          />
        </div>
